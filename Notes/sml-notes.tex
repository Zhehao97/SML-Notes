...
\title{Statistical Machine Learning - Seminar Notes}  
\author{Tsinghua MSE}
\date{\today} 
        ...
        \label{ssub:local_averaging_methods}
            ...
            \begin{figure}[h]
                ...
                \begin{minipage}{0.40\textwidth}
                    \centering
                    \includegraphics[width=\textwidth]{Pics/knn-curve.png}
                \end{minipage}
...
\label{sec:convexification_of_the_risk}
    ...
    \label{sub:convex_surrogates}
        ...
        \begin{figure}[htbp]
            \centering
            \includegraphics[width=.6\textwidth]{Pics/connvex-surrogates.png}
        \end{figure}
    ...
    \label{sub:geometric_interpretation_of_the_support_vector_machine}
        ...
        \begin{figure}[htbp]
            \centering
            \includegraphics[width=.6\textwidth]{Pics/separating-hyperplane.png}
        \end{figure}
    ...
    \label{sub:conditional_surrogate_risk_and_classification_calibration}
        ...
        \begin{figure}[htb]
            \centering
            \includegraphics[width=.7\linewidth]{Pics/classification-calibrated.png}
...
\label{sec:empirical_risk_minimization}
    ...
    \label{sub:approximation_error}
        ...
        \begin{figure}[htb]
            \centering
            \includegraphics[width=.5\textwidth]{Pics/approximation-error.png}
    ...
    \label{sub:estimation_error}
        ...
        \label{ssub:finite_hypothesis_space}
            ...
            \begin{equation*}
                \begin{aligned}
                    \mathbb{P} \left(\mathcal{R}(\hat{f}) - \inf_{f \in \mathcal{F}} \mathcal{R}(f) \geq t \right) &\leq \mathbb{P} \left(2 \sup_{f \in \mathcal{F}} |\hat{\mathcal{R}}(f) - \mathcal{R}(f) | \geq t \right) \\
                    &= \mathbb{P} \left(2 \bigcup_{f \in \mathcal{F}} |\hat{\mathcal{R}}(f) - \mathcal{R}(f) | \geq t \right) \\
            ...
            Finaly, plugging the above result into the Eq.(\ref{eq:expected-unifrom-deviation}), we have with probabability $1 - \delta$
            \begin{equation}
        ...
        \label{ssub:beyond_the_finite_hypothesis_space}
            ...
            \begin{figure}[h]
                ...
                \begin{subfigure}[b]{.45\textwidth}
                    \includegraphics[width=2in]{Pics/covering-number-1.png}
                \end{subfigure}
                \begin{subfigure}[b]{.45\textwidth}
                    \includegraphics[width=2in]{Pics/covering-number-2.png}
                \end{subfigure}
            ...
            \begin{equation*}
                ...
                \end{aligned}
            \end{equation*}
            Using bounds Eq.(\ref{eq:finite-expectation-bounds}) on the expectation of the maximum (bounded random variables are sub-Gaussian), we have
            \begin{equation}
                \begin{aligned}
...
\label{sec:rademacher_complexity}
    ...
    In Section \ref{sec:pac_learn_and_uniform_convergence} we have shown that uniform convergence is a sufficient condition for learnability. In this section, we study the Rademacher complexity, which measures the rate of uniform convergence. We will provide both \textbf{uniform deviation bounds} and \textbf{generailization bounds} based on this measure under distinct consideration. To begin with, let's recall the definition of an $\varepsilon$-representative sample.
    ...
    \label{sub:rademacher_complexity}
        ...
        \begin{theorem}[Empirical Process bonud via Rademacher Complexity]
            ...
            \begin{equation}
                \begin{aligned}
                    ...
                    \sup_{h \in \mathcal{H}} \left[\frac{1}{n} \sum_{i=1}^{n}h(Z_i) - \mathbb{E}[h(Z)] \right] &\leq 2 \hat{R}_{S_n}(\mathcal{H}) + 3 \sqrt{\frac{\log (2 / \delta) }{2n}} \\
                \end{aligned}
                \label{eq:empirical-process-bound-Rademacher-complexity}
            \end{equation}
            \label{thm:empirical-process-bound-Radeamcher-complexity}
        \end{theorem}
        ...
        \begin{corollary}[Generalization Bonud via Rademacher Complexity]
            Suppose for all $h \in \mathcal{H}$, $0 \leq h(Z) \leq 1$. Then, with probability at least $1 - \delta$,
            \begin{equation}
                \begin{aligned}
                    \mathbb{E}[h(Z)] &\leq \frac{1}{n} \sum_{i=1}^{n}h(Z_i) + 2 R_{n}(\mathcal{H}) + \sqrt{\frac{\log (2 / \delta) }{2n}} \\
                    \mathbb{E}[h(Z)] &\leq \frac{1}{n} \sum_{i=1}^{n}h(Z_i) + 2 \hat{R}_{S}(\mathcal{H}) + 3 \sqrt{\frac{\log (2 / \delta) }{2n}} \\
                \end{aligned}
                \label{eq:generlization-bound-Rademacher-complexity}
            \end{equation}
        ...
        \begin{lemma}[Translation Invariant]
            ...
            $$ \hat{R}_{S_n}(\mathcal{H}) = \hat{R}_{S_n}(\mathcal{H}') \quad \text{and} \quad R_{n}(\mathcal{H}) = R_{n}(\mathcal{H}') $$
        \end{lemma}
    ...
    \subsection{Uniform Deviation Bounds for Linear Regression} % (fold)
    \label{sub:uniform_deviation_bounds_for_linear_regression}
    
        \subsubsection{Lipschitz-continuous Losses} % (fold)
        \label{ssub:lipschitz_continuous_losses}
        ...
        % subsubsection lipschitz_continuous_losses (end)

        \subsubsection{Ball-constrained Linear Predictions} % (fold)
        \label{ssub:ball_constrained_linear_predictions}
        ...
        % subsubsection ball_constrained_linear_predictions (end)

        \subsubsection{Putting Things Together} % (fold)
        \label{ssub:putting_things_together}

            With all the elements above (section \ref{ssub:lipschitz_continuous_losses} and \ref{ssub:ball_constrained_linear_predictions}), we can now propose the following general result (where no convexity of the loss function is assumed).
            ...
            \begin{proof}
                Using Proposition \ref{thm:symmetrization-Rademacher-complexity}, equation \ref{eq:Rademacher-complexity-of-hypothesis-class} and \ref{eq:Rademacher-complexity-of-function-class}, we get the desire result.
                Note that this is an uniform deviation bound, which limit the difference between a model's expected risk and its empirical risk uniformly for all models in a hypothesis space.
            \end{proof}
        ...
        % subsubsection putting_things_together (end)

        \subsubsection{From Constrained to Regularized Estimation} % (fold)
        \label{ssub:from_constrained_to_regularized_estimation}
            ...
            \begin{proposition}[Fast Rates for Regularized Objectives]
                Assume a $G$-Lipschitz-continuous convex loss function, linear prediction functions with $\mathcal{F} = \{f_{\theta}(X) = \theta^{\top} \varphi(X) \mid ||\theta||_2 \leq D \}$, where $\mathbb{E}||\varphi(X)||_2^2 \leq R^2$. Let $\hat{\theta}_{\lambda} \in \mathbb{R}^d$ be the minimizer of the regularized empirical risk in equation \ref{eq:regularized-empirical-risk}, then we have a uniform deviation bound:
                \begin{equation*}
                    \mathcal{R}(f_{\hat{\theta}_{\lambda}}) \leq \inf_{\theta \in \mathbb{R}^d} \left\{\mathcal{R}(f_{\theta}) + \frac{\lambda}{2} ||\theta||_2^2 \right\} + \frac{32 G^2 R^2}{\lambda n}     
                \end{equation*} 
            \end{proposition}
        ...
        % subsubsection from_constrained_to_regularized_estimation (end)

    % subsection uniform_deviation_bounds_for_linear_regression (end)

    \subsection{Generalization Bounds for SVM} % (fold)
    \label{sub:generalization_bounds_for_svm}
    
    % subsection generalization_bounds_for_svm (end)
    ...
    \label{sub:vc_dimension}
        ...
        \begin{itemize}
            ...
            \item \textbf{Sine Functions}.
                ...
                \begin{figure}[h]
                    \centering
                    \includegraphics[width=.6\textwidth]{Pics/sine-function.png}
...
\section{Optimization for Machine Learning} % (fold)
\label{sec:optimization_for_machine_learning}
    ...
    \begin{table}[h]
        ...
        \begin{tabularx}{\linewidth}{lXX}
            ...
            \midrule
            Non-smooth  & deterministic: $BD /\sqrt{t}$ & deterministic: $B^2 / (t \mu) $ \\
            ...
            \bottomrule
        \end{tabularx}
        \caption{Above, $L$ is the smoothness constant, $\mu$ the strong convexity constant, $B$ the Lipschitz constant and $D$ the distance to optimum at initialization}
    ...
    \label{sub:optimization_in_machine_learning}
        ...
        \end{equation*}
        where $l: \mathbb{Y} \times \mathcal{Y} \mapsto \mathbb{R}$ is a loss function. This loss is typlically convex, which is considered as a weak assumption. 
        In the empirical risk minimization (ERM) approach desribed in Section \ref{sec:empirical_risk_minimization}, for a parametrization $\{f_\theta\}_{\theta \in \mathbb{R}^d} $ and a regularizer $\Omega: \mathbb{R}^d \mapsto \mathbb{R}$ (e.g. $\Omega(\theta) = ||\theta||_2^2$ or $\Omega(\theta) = ||\theta||_1$ ), this require to minimize
        \begin{equation}
        ...
        In general, the minimizer has no closed form. Even when it has one (e.g. linear predictor and square loss), it could be expensive to compute for large problems. We thus resort to iterative algorithms. If the algorithm return $\hat{\theta}$ and let $\theta^* \in \argmin_{\theta} \mathcal{R}(f_{\theta})$, we can decompose the estimation errpr into (the approximation error due to the selection of hypothesis class $\Theta$ is ignored)

        \begin{equation*}
            \mathcal{R}(f_{\hat{\theta}}) - \inf_{\theta \in \mathbb{R}^d} \mathcal{R}(f_{\theta}) = \underbrace{ \big\{\mathcal{R}(f_{\hat{\theta}} ) - \hat{\mathcal{R}}(f_{\hat{\theta}} ) \big\} }_{\leq \text{estimation error}} + \underbrace{ \big\{\hat{\mathcal{R}}(f_{\hat{\theta}}) - \hat{\mathcal{R}}(f_{\theta^*} ) \big\} }_{\leq \text{optimization error}} + \underbrace{ \big\{\hat{\mathcal{R}}(f_{\theta^*}) - \mathcal{R}(f_{\theta^*} ) \big\} }_{\leq \text{estimation error}}
        \end{equation*}
        ...
        In the following content, we use the notation $\theta^{*}$ for the minimizer of the expected risk and the notation $\theta_{\star}$ (not $\hat{\theta}$) for the minimizer of the empirical (potentially regularized) risk.
    
    % subsection optimization_in_machine_learning (end)

    \subsection{Gradient Descent on Smooth Problems} % (fold)
    \label{sub:gradient_descent_on_smooth_problems}
    
        Suppose we want to solve, for a function $F: \mathbb{R}^d \mapsto \mathbb{R}$, the optimization problem
        \begin{equation*}
            \min_{\theta \in \mathbb{R}^d} \ F(\theta)              
        \end{equation*}        
        In this section, we do not assume that function $F$ is regularized empirical risk. We further assume that we are given access to certian oracles -- the $k$th-order oracle corresponds to the access to $\theta \mapsto (F(\theta), F'(\theta), \dots, F^{(k)}(\theta) )$, that is all partial derivatives up to order $k$. All algorithms will call these oracles and thus their computational complexity will depend directly on the complexity of this oracle.
        ...
        \end{equation}
        for a well (potentially adaptively) chosen step-size sequence $(\gamma_t)_{t \geq 1}$. There are many ways to choose the step-size $\gamma_t$, either constant, either decaying, either through a line search. In practice, using some form of line search is strongly advantageous and is implemented in most applicaiton. In this section, to simplify the algorithms and proofs, we will focus on constant or iteration-dependent step-size.
        ...
        \subsubsection{Analysis of GD for ordinary least squares} % (fold)
        \label{ssub:analysis_of_gd_for_ordinary_least_squares}
        
            We first start with the simplest example, namely the quadratic convex functions (which is strongly convex), which is just the loss function in ordinary least squares. Let $\Phi \in \mathbb{R}^{n \times d}$ be the design matrix, and $y \in \mathbb{R}^n$ the vector of responses. Least-squares estimation amoutns to finding a minimizer $\theta_{\star}$ of

            \begin{equation*}
                F(\theta) = \frac{1}{2n} ||\Phi \theta - y||_2^2 = \frac{1}{2n} (y - \Phi \theta)^{\top} (y - \Phi \theta)
            \end{equation*}
            The gradient of $F$ is $F'(\theta) = \frac{1}{n} \Phi^{\top}(\Phi \theta - y) = \frac{1}{n} \Phi^{\top} \Phi \theta - \frac{1}{n} \Phi^{\top} y $. Thus, denoting $H = \frac{1}{n} \Phi^{\top} \Phi \in \mathbb{R}^{d \times d}$ the Hessian matrix (with respect to $\theta$), the minimizers $\theta_{\star}$ are characterized by
            \begin{equation}
                F'(\theta_{\star}) = 0 \quad \Rightarrow \quad H \theta_{\star} = \frac{1}{n} \Phi^{\top} y
            \end{equation}
            Since $\frac{1}{n} \Phi^{\top}y \in \mathbb{R}^d$ is in the range space of $H$, there is always a minimizer, but unless $H$ is invertible, the minimizer is not unique. But all minimizers $\theta_{\star}$ have the same function value $F(\theta_{\star})$, and we have, from a simple Taylor expansion
            \begin{equation*}
                \begin{aligned}
                    F(\theta) - F(\theta_{\star}) &= F'(\theta_{\star})^{\top}(\theta - \theta_{\star}) + \frac{1}{2} (\theta - \theta_{\star})^{\top} H (\theta - \theta_{\star}) \\ 
                    &= \frac{1}{2} (\theta - \theta_{\star})^{\top} H (\theta - \theta_{\star}) \\
                \end{aligned}
            \end{equation*}
            ...
            \vspace{1em}
            \noindent
            \textbf{Closed-form expression}. Gradient descent iterates with fixed step-size $\gamma_t = \gamma$ can be computed in closed form:
            \begin{equation*}
                \begin{aligned}
                    \theta_t = \theta_{t-1} - \gamma F'(\theta_{t-1}) &= \theta_{t-1} - \gamma \left[\frac{1}{n} \Phi^{\top}(\Phi \theta_{t - 1} - y) \right] \\
                    &= \theta_{t-1} - \gamma H(\theta_{t-1} - \theta_{\star}) \\
                \end{aligned}
            \end{equation*}
            leading to 
            \begin{equation*}
                \theta_t - \theta_{\star} = \theta_{t - 1} - \theta_{\star} - \gamma H(\theta_{t - 1} - \theta^{*}) = (I - \gamma H) (\theta_{t - 1} - \theta_{\star})
            \end{equation*}
            ...
            \end{equation*}
            We can now look at various measures of performace:
            \begin{equation*}
                \begin{aligned}
                    ||\theta_t - \theta_{\star}||_2^2 &= (\theta_0 - \theta_{\star})^{\top} (I - \gamma H)^{2t} (\theta_0 - \theta_{\star}) \\
                    F(\theta_t) - F(\theta_{\star}) &= \frac{1}{2} (\theta_0 - \theta_{\star} )^{\top} (I - \gamma H)^{2t} H (\theta_0 - \theta_{\star})
                \end{aligned}
            \end{equation*}
            Note that the second equality holds by applying $H \theta_{\star} = \frac{1}{n} \Phi^{\top} y$.

        % subsubsection analysis_of_gd_for_ordinary_least_squares (end)

        \subsubsection{Analysis of GD for strongly and smooth functions} % (fold)
        \label{ssub:analysis_of_gd_for_strongly_and_smooth_functions}

            The analysis of optimization algorithms requires assumptions on the objective functions, like the ones introduced in this section. From these assumptions, additional properties are derived (typically inequalities), and then most convergence proofs look for a "Lyapunov function" that goes down along the iterations. More precisely, if $V: \mathbb{R}^d \mapsto \mathbb{R}_{+}$ is such that $V(\theta_t) \leq (1 - \alpha) V(\theta_{t - 1})$, then $V(\theta_t) \leq (1 - \alpha)^t V(\theta_0)$ and we obtain linear convergence rate. The art is then to find the appropriate Lynapunov function.

            \vspace{1em}
            We first consider an assumption allowing exponential convergence rates.
            \begin{definition}[Convex function]
                A differentiable function $F: \mathbb{R}^d \mapsto \mathbb{R}$ is said to be convex if and only if 
                \begin{equation}
                    F(\eta) \geq F(\theta) + F'(\theta)^{\top} (\eta - \theta), \quad \forall \ \eta, \theta \in \mathbb{R}^d
                    \label{eq:convexity}
                \end{equation}
            ...
            \begin{definition}[Strong convexity]
                A differentiable function $F$ is said $\mu$-strongly convex, with $m > 0$, if and only if
                \begin{equation}
                    F(\eta) \geq F(\theta) + F'(\theta)^{\top}(\eta - \theta) + \frac{\mu}{2} ||\eta - \theta||_2^2
                    \label{eq:strong-convexity}
                \end{equation}
                for all $\eta, \theta \in \mathbb{R}^d$. For twice differentiable functions, this is equivalent to $F'' \succeq m I$.
            \end{definition}
            ...
            \begin{figure}[h]
                \centering
                \includegraphics[width=.6\linewidth]{Pics/strongly-convex.png}
                \caption{Strong convexity}
                \label{fig:strongly-convex}
            \end{figure}

            \begin{lemma}[Lojasiewics inequality]
                If $F$ is differentiable and $m$-strongly convex with unique minimizer $\theta_{\star}$, then we have:
                \begin{equation*}
                    || F'(\theta) ||_2^2 \geq 2 \mu (F(\theta) - F(\theta_{\star}))
                \end{equation*}
                \label{lm:lojasiewics-inequality}
            \end{lemma}

            \begin{proof}
                The right-hand side of Eq.(\ref{eq:strong-convexity}) is strongly convex in $\eta$ and minimized with $\eta = \theta - \frac{1}{\mu } F'(\theta)$. Plugging this value into the bound and letting $\eta = \theta_{\star}$, we get
                \begin{equation*}
            ...
            \begin{definition}[Smoothness]
                A differentiable function $F$ is said $L$-smooth if and only if
                \begin{equation}
                    |F(\eta) - F(\theta) - F'(\theta)^{\top}(\eta - \theta) | \leq \frac{L}{2} ||\eta - \theta||_2^2 \quad \forall \ \theta, \eta \in \mathbb{R}^d
                    \label{eq:smoothness}
                \end{equation}
            ...
            \begin{equation*}
                F(\eta) \leq F(\theta) + F'(\theta) + F'(\theta)^{\top}(\eta - \theta) + \frac{L}{2} ||\eta - \theta||_2^2
            \end{equation*}
            For machine learning problems with linear predictions and smooth losses (squared or logistic), we have smooth problems. If we use a squared $l_2$-regularizer $\frac{\mu}{2}||\cdot||_2^2$, we get a $\mu$-strongly convex problem. Note that when using regularization, the value of $\mu$ decays with $n$, typically between $1/n$ and $1/\sqrt{n}$, leading to condition numbers $\kappa$ bewtween $\sqrt{n}$ and $n$.

            \begin{figure}[h]
                \centering
                \includegraphics[width=.6\linewidth]{Pics/smoothness.png}
                \caption{$L$-smoothness}
                \label{fig:smoothness}
            \end{figure}

            \begin{figure}[h]
                \centering
                \includegraphics[width=.6\linewidth]{Pics/condition-number.png}
                \caption{When a function is both smooth and strongly convex, we denote by $\kappa = L/\mu \geq 1$ its condition number, and the condition number impacts the shapes of the level sets}
                \label{fig:condition-number}
            \end{figure}

            \begin{figure}[h]
                \centering
                \includegraphics[width=.6\linewidth]{Pics/condition-number-steepest.png}
                \caption{The performance of gradient descent will depend on this condition number (see the steepest descent, i.e., gradient descent with exact line search): with small condition number (left), we get fast convergence, while for a large condition number (right), we get oscillations}
                \label{fig:condition-number-steepest}
            \end{figure}

            In the next theorem, we show that gradient descent converges exponentially for such smooth and strongly-convex problems.
            
            \begin{theorem}[Convergence of GD for smooth strongly-convex functions]
                Assume that $F$ is $L$-smooth and $\mu$-strongly convex. Choosing step-size $\gamma_t = 1/L$, the iterates $(\theta_t)_{t \geq 0}$ of Gradient Descent on $F$ satisfy
                \begin{equation*}
            ...
            \begin{proof}
                By the smoothness inequality in Eq.(\ref{eq:smoothness}) applied to $\theta_{t-1}$ and $\theta_{t-1} - F'(\theta_{t-1}) / L$, we have the following descent property, with $\gamma_t = 1/L$,
                \begin{equation}
                    \begin{aligned}
                        F(\theta_t) = F(\theta_t - F'(\theta_{t-1})/L) &\leq F(\theta_{t-1}) + F'(\theta_{t-1})^{\top} (-F'(\theta_{t-1}) / L) + \frac{L}{2} ||- F'(\theta_{t-1}) / L||_2^2 \\
                        &= F(\theta_{t-1}) - \frac{1}{2L} ||F'(\theta_{t-1})||_2^2 \\
                    \end{aligned}
                    \label{eq:iteration-inequality}
                \end{equation}
                Adding $-F(\theta_{\star})$ on both sides, we get
                \begin{equation*}
                    F(\theta_t) - F(\theta_{\star}) \leq (F(\theta_{t-1}) - F(\theta_{\star})) - \frac{1}{2L} || F'(\theta_{t-1}) ||_2^2
                \end{equation*}
                Using Lojasiewicz inequality (Lemma \ref{lm:lojasiewics-inequality}), it follows
                \begin{equation*}
                    F(\theta_t) - F(\theta_{\star}) \leq (1 - \mu/L) \cdot (F(\theta_{t-1}) - F(\theta_{\star}) ) \leq e^{- \mu/L} \cdot (F(\theta_{t-1}) - F(\theta_{\star}) )
                \end{equation*}
            ...
            We can make the following observations:
            \begin{itemize}
                \item As mentioned before, we necessarily have $\mu \leq L$; the ratio $\kappa := L/\mu$ is called the condition number

                \item If we only assume that the function is smooth and convex (not strongly convex), then Gradient Descent with constant step-size $\gamma = 1/L$ also converges when a minimizer exists, but at a slower rate $\mathcal{O}(1/t)$. More details in next section.

                \item Choosing the step-size only requires an upper bound $L$ on the smoothness constant (in case it is over-estimated, the convergence rate only degrades slightly)

            \end{itemize}

        % subsubsection analysis_of_gd_for_strongly_and_smooth_functions (end)

        \subsubsection{Analysis of GD for convex and smooth functions} % (fold)
        \label{ssub:analysis_of_gd_for_convex_and_smooth_functions}
            ...
            \begin{proposition}[co-coercivity]
                If $F$ is a convex $L$-smooth function on $\mathbb{R}^d$, then for all $\theta, \eta \in \mathbb{R}^d$, we have
                \begin{equation*}
            ...
            \begin{proof}
                Define $H(\theta) = F(\theta) - F'(\eta)^{\top} \theta$. The function $H: \mathbb{R}^d \mapsto \mathbb{R}$ is convex in $\theta$ with global minimum at $\eta$, and is also $L$-smooth. Now, we can apply the property of $L$-smoothness in Eq.(\ref{eq:smoothness}) and get
                
                \begin{equation*}
                    \begin{aligned}
                        H(\eta) \leq H(\theta - H'(\theta) / L) &\leq H(\theta) + H'(\theta)^{\top}(- H'(\theta) / L) + \frac{L}{2} ||- H'(\theta) / L||_2^2 \\
                        &= H(\theta) - \frac{1}{2L} ||H'(\theta)||_2^2
                    \end{aligned}
                \end{equation*}
                This leads to 
            ...
            \begin{theorem}[Convergence of GD for smooth convex functions]
                Assume that $F$ is $L$-smooth and convex, with a global minimizer $\theta_{\star}$. Choosing step-size $\gamma_t = 1/L$, the iterates $(\theta_t)_{t \geq 0}$ of Gradient Descent on $F$ satisfy
                \begin{equation*}
                    F(\theta_t) - F(\theta_{\star}) \leq \frac{L}{2t} ||\theta_0 - \theta_{\star} ||_2^2
                \end{equation*}
            \end{theorem}

            \begin{proof}
                The Lyapunov function that we will choose is
                \begin{equation}
                    V_t(\theta_t) = t (F(\theta_t) - F(\theta_{\star})) + \frac{L}{2}||\theta_t - \theta_{\star}||_2^2    
                \end{equation}
                and our goal is to show that it decays along iterations. We can split the difference in Lyapunov functions in three terms
                \begin{equation*}
                    V_t(\theta_t) - V_{t-1}(\theta_{t-1}) = \underbrace{t F(\theta_t) - t F(\theta_{t-1}) }_{\text{part I}} + \underbrace{ F(\theta_{t-1}) - F(\theta_{\star}) }_{\text{part II}} + \underbrace{ \frac{L}{2} ||\theta_t - \theta_{\star}||_2^2 - \frac{L}{2}||\theta_{t-1} - \theta_{\star}||_2^2 }_{\text{part III}}
                \end{equation*}
                We bound these three parts separately:
                \begin{itemize}
                    \item From Eq.(\ref{eq:iteration-inequality}), we have
                        \begin{equation*}
                            F(\theta_t) - F(\theta_{t-1}) \leq -\frac{1}{2L} ||F'(\theta_{t-1})||_2^2
                        \end{equation*}

                    \item For a convex function, we have
                        \begin{equation*}
                            F(\theta_{\star}) \geq F(\theta_{t-1}) + F'(\theta_{t-1})^{\top} (\theta_{\star} - \theta_{t-1}) \quad \Rightarrow \quad F(\theta_{t-1}) - F(\theta_{\star}) \leq F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star})
                        \end{equation*}

                    \item Expanding the difference of square, we have
                        \begin{equation*}
                            \begin{aligned}
                                ||\theta_{t} - \theta_{\star}||_2^2 - ||\theta_{t-1} - \theta_{\star}||_2^2 &= ||\theta_{t-1} - \gamma_t - \theta_{\star} ||_2^2 - - ||\theta_{t-1} - \theta_{\star}||_2^2 \\
                                &= - 2 \gamma_t F'(\theta_{t-1})^{\top}(\theta_{t-1} - \theta_{\star} ) + \gamma_t^2 ||F'(\theta_{t-1})||_2^2
                            \end{aligned}
                        \end{equation*}
                \end{itemize}
                ...
                \begin{equation*}
                    \begin{aligned}
                        V_t(\theta_t) - V_{t-1}(\theta_{t-1}) &\leq - \frac{t}{2L} ||F'(\theta_{t-1})||_2^2 + F'(\theta_{t-1})^{\top}(\theta_{t-1} - \theta_{\star}) - F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) + \frac{1}{2L} ||F'(\theta_{t-1}) ||_2^2 \\
                        &= \frac{1 - t}{2L} ||F'(\theta_{t-1})||_2^2 \leq 0
                    \end{aligned}
                \end{equation*}
                which leads to
                \begin{equation*}
                    t(F(\theta_t) - F(\theta_{\star}) ) \leq V_t(\theta_t) \leq V_0(\theta_0) = \frac{L}{2} ||\theta_0 - \theta_{\star}||_2^2
                \end{equation*}
                and thus $F(\theta_t) - F(\theta_{\star}) \leq \frac{L}{2t} ||\theta_0 - \theta_{\star}||_2^2$.

            \end{proof}

            The proof above is on purpose mysterious: the choice of Lyapunov function seems arbitrary at first, but all inequalities lead to nice cancellations. These proofs are sometimes hard to design. See \cite{bach2021learning} for more interesting details.

        % subsubsection analysis_of_gd_for_convex_and_smooth_functions (end)

        \subsubsection{Beyond Gradient Descent} % (fold)
        \label{ssub:beyond_gradient_descent}

            While gradient descent is the simplest algorithm with a simple analysis, there are multiple extensions we will here briefly mention (more details see \cite{nesterov2018lectures})

            \begin{itemize}
                \item \textbf{Nesterov acceleration}. For convex functions, a simple modification of gradient descent allows to obtain better convergence rates. The algorithm is as follows:
                    \begin{equation*}
                        \begin{aligned}
                ...
                \item \textbf{Newton method}. Given $\theta_{t-1}$, the Newton method minimizes the second-order Tarylor expansion around $\theta_{t-1}$:
                    \begin{equation*}
                        F(\theta_t) \approx F(\theta_{t-1}) + F'(\theta_{t-1})^{\top} (\theta - \theta_{t-1}) + \frac{1}{2} (\theta - \theta_{t-1})^{\top} F''(\theta_{t-1})^{\top} (\theta - \theta_{t-1})
                    \end{equation*}
                    which leads to the descent algorithm{}
                    \begin{equation}
                        \theta_t = \theta_{t-1} - F''(\theta_{t-1})^{-1} F'(\theta_{t-1}) 
                    \end{equation}
                    This is an expensive iteration, as the running-time compelxity is $\mathcal{O}(d^3)$ in general to solve the linear system (matrix inverse). It leads to local quadratic rate convergence. See \cite{boyd2004convex} for more details, and for condition for global convergence.
                ...
                \item \textbf{Proximal gradient descent}. Many optimization problem are said "composite", that is, the objective function $F$ is the sum of a smooth function $G$ and a non-smooth function $H$ (such as norm). It turns out that a simple modification of gradient descent allows to benefit from the fast convergence rates of smooth optimization.
                    ...
                    \begin{equation*}
                        \theta_t = \argmin_{\theta \in \mathbb{R}^d} \ G(\theta_{t-1}) + G'(\theta_{t-1})^{\top} (\theta - \theta_{t-1}) + \frac{L}{2} ||\theta - \theta_{t-1}||_2^2
                    \end{equation*}
                    where, for a $L$-smooth function $G$, the objective function above is an upper-bound of $G(\theta)$, which is tight at $\theta_{t-1}$. While this reformulation does not bring much for gradient descent, we can extend this to the composite problem, and consider the iteration
                    \begin{equation*}
                        \theta_t = \argmin_{\theta \in \mathbb{R}^d} \ G(\theta_{t-1}) + G'(\theta_{t-1})^{\top} (\theta - \theta_{t-1}) + \frac{L}{2}||\theta - \theta_{t-1}||_2^2 + H(\theta)
                    \end{equation*}
                    where $H$ is left as what it is. The crux is to be able to compute the step above, that is minimize with respect to $\theta$ functions of the form $\frac{L}{2}||\theta - \theta_{t-1}||_2^2 + H(\theta)$. When $H$ is the indicator function of a convex set (which is equal to $0$ inside the set, and $+ \infty$ otherwise), we get \textbf{projected gradient descent}. When $H$ is the $l_1$-norm, that is $H = \lambda ||\cdot||_1$, this can be shown to be soft-thresholding step, as for coordinate $\theta_i = \eta_i / |\eta_i| (|\eta_i| - \lambda/L)^{+}$.

                    It turns out that the convergence rates for G + H are the same as smooth optimization, with potential acceleration (\cite{nesterov2013gradient}).

            \end{itemize}
        
        % subsubsection beyond_gradient_descent (end)

    % subsection gradient_descent_on_smooth_problems (end)

    \subsection{Gradient Methods on Non-smooth Problems} % (fold)
    \label{sub:gradient_methods_on_non_smooth_problems}
        ...
        \begin{definition}[Lipschitz-continuous function]
            A function $F: \mathbb{R}^d \mapsto \mathbb{R}$ is said $B$-Lipschitz-continuous if and only if
            \begin{equation*}
        ...
        Without additional assumptions, this setting is usually refered to as non-smooth optimization. Now we can apply non-smooth optimization to objective functions which are not differentiable (such as the hinge loss). For convex Lipschitz-continuous objectives, the function is differentiable almost everywhere. In points where it is not, then one can define the set of slopes of lower-bounding tangents as the \textit{subdifferential}, and any element of it as a \textit{subgradient}.
        The Gradient Descent iteration is then meant as using any subgradient instead of $F'(\theta_{t-1})$, formally,
        \begin{equation*}
        ...
        \begin{theorem}[Convergence of the Subgradient Method]
            Assume that $F$ is convex, $B$-Lipschitz-continuous, and admits a minimizer $\theta_{\star}$ that satisfies $||\theta_{\star} - \theta_0||_2 \leq D$. By choosing $\gamma_t = \frac{D}{B \sqrt{t}}$ then the iterates $(\theta_t)_{t \geq 0}$ of Subgradient Method on $F$ satisfy
            \begin{equation*}
                \min_{0 \leq s \leq t-1} \ F(\theta_s) - F(\theta_{\star}) \leq BD \frac{2 + \log t}{2 \sqrt{t}}
            \end{equation*}
            \label{thm:convergence-GD-non-smooth}
        \end{theorem}

        \begin{proof}
            We look at how $\theta_t$ approaches $\theta_{\star}$, that is, we try to use $||\theta_t - \theta_{\star}||_2^2$ as a Lyapunov function. We have:
            \begin{equation*}
                \begin{aligned}
                    ||\theta_t - \theta_{\star}||_2^2 &= ||\theta_{t-1} - \gamma_t F'(\theta_{t-1}) - \theta_{\star}||_2^2 \\
                    &= ||\theta_{t-1} - \theta_{\star}||_2^2 - 2 \gamma_t F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) + \gamma_t^2 ||F'(\theta_{t-1]})||_2^2
                \end{aligned}
            \end{equation*}
            Combining this with the convexity inequality $F(\theta_{t-1}) - F(\theta_{\star}) \leq F'(\theta_{t-1})^{\top}(\theta_{t-1} - \theta_{\star})$ from Eq.(\ref{eq:convexity}), we have (also using the boundedness of gradients/Lipschiz-continuous)
            \begin{equation*}
                ||\theta_t - \theta_{\star}||_2^2 \leq ||\theta_{t-1} - \theta_{\star}||_2^2 - 2 \gamma_t [F(\theta_{t-1}) - F(\theta_{\star}) ] + \gamma_t^2 B^2
            \end{equation*}
            and thus, by isolating the distance to optimum in function values:
            \begin{equation}
                \gamma_t (F(\theta_{t-1}) - F(\theta_{\star}) ) \leq \frac{1}{2} \left(||\theta_{t-1} - \theta_{\star}||_2^2 - ||\theta_t - \theta_{\star}||_2^2 \right) + \frac{1}{2} \gamma_t^2 B^2
                \label{eq:gradient-methods-non-smooth}
            \end{equation}
            It is sufficient to sum these inequalities up to $t$ and drop the term $-||\theta_{t+1} - \theta_{\star}||_2^2$ to get, for any $\theta_{\star} \in \mathbb{R}^d$,
            \begin{equation*}
                \frac{1}{\sum_{s=1}^{t} \gamma_s} \sum_{s=1}^{t} \gamma_s (F(\theta_{s-1}) - F(\theta_{\star}) ) \leq \frac{||\theta_0 - \theta_{\star}||_2^2}{2 \sum_{s=1}^{t} \gamma_s} + B^2 \frac{\sum_{s=1}^{t} \gamma_s^2}{2 \sum_{s=1}^{t} \gamma_s}
            \end{equation*}
            where the left-hand side is trivially larger than $\min_{0 \leq s \leq t-1} (F(\theta_s) - F(\theta_{\star}) ) $ as well as $F(\bar{\theta}_t) - F(\theta_{\star})$. Here we denote $\bar{\theta}_t = (\sum_{s=1}^{t} \gamma_s \theta_{s-1}) / (\sum_{s=1}^{t} \gamma_s) $ by Jensen's inequality.
            ...
            we get the bound
            \begin{equation*}
                \min_{0 \leq s \leq t-1} (F(\theta_s) - F(\theta_{\star}) ) \leq \frac{1}{2 \tau \sqrt{t}} \left(D^2 + \tau^2 B^2 (1 + \log t) \right)
            \end{equation*}
        ...
        \begin{itemize}
            \item There is no need to know in advance an uppper-bound $D$ on the distance to optimum, we then get with the same step-size $\gamma_t = \frac{D}{B \sqrt{t}} $ a rate of the form $\frac{BD}{2 \sqrt{t}} \left(\frac{||\theta_0 - \theta_{\star}||_2^2 }{D^2} + (1 + \log t) \right) $

            \item The algorithm applies to \textbf{constrained minimization} over a convex set, by inserting a proection step at each iteration (the proof, which is using the contractivity of orthogonal projections, is essentially the same)

            \item The algorithm applies to \textbf{non-differentiable} convex and Lipschitz objective functions using sub-gradients (any vector satisfying Eq.(\ref{eq:convexity} in place of $F'(\theta_t)$)

            \item The algorithm can be applied to \textbf{non-Euclidean geometries}, where we consider bounds on the iterates or the gradient with different quantities. This can be done using "mirror descent" framework, and for instance can be applied to obtain multiplicative updates

            \item Often the uniformly averaged iterate, $\frac{1}{t} \sum_{s=0}^{t-1} \theta_s$ is used. Convergence rates without $\log t$ factor can be obtained using Abel summation formula

            \item Stochastic gradients method can be used, as presented below (one interpretation is that the subgradient method is so slow that it is robust to noisy gradients)

        \end{itemize}
        More details see \cite{bach2021learning}.

    % subsection gradient_methods_on_non_smooth_problems (end)
    ...
    \label{sub:stochastic_gradient_descent}
        ...
        \begin{itemize}
            \item we need to condition over $\theta_{t-1}$ because $\theta_{t-1}$ encapsulates all the randomness due to past iterations, and we only require "fresh" randomness at time $t$

            \item this unbiasedness does not need to be coupled with a vanishing variance: while there are always errors in the gradient, the use of a decreasing step-size will ensure convergence. If the noise in the gradient is not unbiased, then we only get convergence if the noise magnitudes go to zero.
        ...
        \begin{equation}
            ...
            \label{eq:stochasitc-gradient-descent}
        \end{equation}
        ...
        \begin{enumerate}
            \item \textbf{Empirical risk minimization}. If $F(\theta) = \frac{1}{n} \sum_{i=1}^{n} l(y_i, f_{\theta}(x_i)) $, then at iteration $t$, we can choose uniformly at random $i(t) \in \{1, \dots, n\}$ and define $g_t$ as the gradient of $\theta \mapsto l(y_{i(t)}, f_(\theta)(x_{i(t)}) ) $. There exists "mini-batch" variants where at each iteration, the gradient is averaged over a random subset of the indices -- we then reduce the variance of the gradient estimate, but we use more gradients, and thus more running time. We then coverge to a minimizer $\theta_{\star}$ of the empirical risk.

                Note here that since we sample with replacement, a given function will be selected serveral times.

            \item \textbf{Expected risk minimization}. If $F(\theta) = \mathbb{E}[l(y, f_{\theta}(x)) ]$, then at iteration $t$ we can take a fresh sample $(x_t, y_t)$ and define $g_t$ as the gradient of $\theta \mapsto l(y_t, f_{\theta}(x_t))$, for which, if we swap the order of expectation and differentiation, we get the unbiasness. 

                Note here that to presever the unbiasedness, only a single pass is allowed (otherwise, this would create dependdencies that would break it).

                Here, we directly minimize the (generalization) risk. The counterpart is that if we only have $n$ samples, then we can only run $n$ SGD iterations, and when $n$ grows, the iterates will converge to a minimizer $\theta^{*}$ of the expected risk.
        ...
        Under the same usual assumptions on the objective functions, to study SGD, we need extra assumptions:
        \begin{itemize}
            \item (A1) unbiased gradient: $\forall \ t, \mathbb{E}[g_t(\theta_{t-1}) \mid \theta_{t-1}] = F'(\theta_{t-1})$

            \item (A2) bounded gradient: $\forall \ t, ||g_t(\theta_{t-1})||_2^2 \leq B^2$ almost surely
        ...
        \begin{theorem}[Convergence of SGD]
            Assume that $F$ is convex, $B$-Lipschitz and admits a minimizer $\theta_{\star}$ and admits a minimizer $\theta_{\star}$ that satisfies $||\theta_{\star} - \theta_0|| \leq D$. Assume that the stochastic gradients satisfy Assumption A1 - A2. Then, choosing $\gamma_t = D/ (B \sqrt{t})$, the iterates $(\theta_t)_{t\geq 0}$ of SGD on $F$ satisfy
            \begin{equation*}
                \mathbb{E}\left[ F(\bar{\theta}_t) - F(\theta_{\star}) \right] \leq BD \frac{2 + \log t}{2 \sqrt{t}}
            \end{equation*}
            where $\bar{\theta}_t = (\sum_{s=1}^{t} \gamma_s \theta_{s-1}) / (\sum_{s=1}^{t} \gamma_s )$.
            \label{thm:convergence-SGD-non-smooth}
        \end{theorem}

        \begin{proof}
            We follow essentially the same proof as in the determinisitc gradient method case, adding some expectations at well chosen places. We have:
            \begin{equation*}
                \begin{aligned}
                    \mathbb{E}\left[ ||\theta_t - \theta_{\star}||_2^2 \right] &= \mathbb{E}\left[ ||\theta_{t-1} - \gamma_t F'(\theta_{t-1}) - \theta_{\star}||_2^2 \right] \\
                    &= \mathbb{E}\left[ ||\theta_{t-1} - \theta_{\star}||_2^2 \right] - 2 \gamma_t \mathbb{E}\left[ F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) \right] + \gamma_t^2 \mathbb{E}\left[ ||F'(\theta_{t-1]})||_2^2 \right] \\
                \end{aligned}
            \end{equation*}
            We can then compute the expectation of the middle term as:
            \begin{equation*}
                \begin{aligned}
                    \mathbb{E}\left[g_t(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) \right] &= \mathbb{E}\left[\mathbb{E}\left[g_t(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) \mid \theta_{t-1} \right] \right] \\
                    &= \mathbb{E}\left[ \mathbb{E}\left[g_t(\theta_{t-1}) \mid \theta_{t-1} \right]^{\top} (\theta_{t-1} - \theta_{\star}) \right] \\
                    &= \mathbb{E}\left[F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) \right] \\
                \end{aligned}
            \end{equation*}
            This leads to
            \begin{equation*}
                \mathbb{E}\left[ ||\theta_t - \theta_{\star}||_2^2 \right] \leq \mathbb{E}\left[ ||\theta_{t-1} - \theta_{\star}||_2^2 \right] - 2 \gamma_t \mathbb{E}\left[F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star}) \right] + \gamma_t^2 B^2
            \end{equation*}
            Thus, combining with the convexity inequality $F(\theta_{t-1}) - F(\theta_{\star}) \leq F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta_{\star})$ from Eq.(\ref{eq:convexity}), we get (also using the boundedness of gradients/Lipschiz-continuous)
            \begin{equation*}
                \mathbb{E}\left[ ||\theta_t - \theta_{\star}||_2^2 \right] \leq \mathbb{E}\left[ ||\theta_{t-1} - \theta_{\star}||_2^2 \right] - 2 \gamma_t \mathbb{E} \left[F(\theta_{t-1}) - F(\theta_{\star}) \right] + \gamma_t^2 B^2
            \end{equation*}
            which implies
            \begin{equation}
                 \gamma_t \mathbb{E} \left[F(\theta_{t-1}) - F(\theta_{\star}) \right] \leq \frac{1}{2} \left(\mathbb{E}[||\theta_{t-1} - \theta_{\star}||_2^2] - \mathbb{E}[||\theta_{t} - \theta_{\star}||_2^2] \right) + \frac{1}{2} \gamma_t^2 B^2
                 \label{eq:stochastic-gradient-methods-non-smooth}
            \end{equation}
            Except for the expectations, this is the same bound as Eq.(\ref{eq:gradient-methods-non-smooth}), so similarily, it is sufficient to sum these inequalities up to $t$ and drop the term $-||\theta_{t+1} - \theta_{\star}||_2^2$ to get, for any $\theta_{\star} \in \mathbb{R}^d$,
            \begin{equation*}
                \frac{1}{\sum_{s=1}^{t} \gamma_s} \sum_{s=1}^{t} \gamma_s \mathbb{E} [ F(\theta_{s-1}) - F(\theta_{\star}) ] \leq \frac{\mathbb{E}[ ||\theta_0 - \theta_{\star}||_2^2] }{2 \sum_{s=1}^{t} \gamma_s} + B^2 \frac{\sum_{s=1}^{t} \gamma_s^2}{2 \sum_{s=1}^{t} \gamma_s}
            \end{equation*}
            Since $F(\theta)$ is convex, the left-hand side is trivially larger than $\mathbb{E}[F(\bar{\theta}_t) - F(\theta_{\star})]$ where we denote $\bar{\theta}_t = (\sum_{s=1}^{t} \gamma_s \theta_{s-1}) / (\sum_{s=1}^{t} \gamma_s) $ by Jensen's inequality.
            ...
            we get the bound
            \begin{equation*}
                \mathbb{E}[F(\bar{\theta}_t) - F(\theta_{\star})] \leq \frac{1}{2 \tau \sqrt{t}} \left(D^2 + \tau^2 B^2 (1 + \log t) \right)
            \end{equation*}
            Choosing $\tau = D/B$ leads to the desired results (which is suggested by optimizing the previous bound when $\log t = 0$).
        ...
        We can make the following observations:
        \begin{itemize}
            \item  Averaging of iterates is often performed after a certain number of iterations (e.g., one pass over the data when doing multiple passes): this speeds up the algorithms by forgetting initial conditions faster

            \item Many authors consider the projected version of the algorithm where after the gradient step, we orthogonally project onto the ball of radius D and center θ0. The bound is then \textbf{exactly the same}

            \item The result that we obtain, when applied to single pass SGD, is a generalization bound, that is, after the $n$ iterations, we have an excess risk proportional to $1/\sqrt{n}$, corresponding to the excess risk compared to the best predictor $f_{\theta}$.

        \end{itemize}

        \noindent
        \textbf{SGD or gradient descent on the empirical risk?} As seen above, the number of iterations to reach a given precision will be larger for stochastic gradient descent, but with a complexity which is typically n times faster. Thus, for high precision, that is low values of $(\theta) − F(\theta_{\star})$ (which is not needed for machine learning), the number of iterations of SGD may become prohibitively large, and deterministic full gradient descent could be preferred. However, for low precision and large $n$, SGD is the method of choice (see also recent improvements.

    
    % subsection stochastic_gradient_descent (end)

% section optimization_for_machine_learning (end)
...
\label{sec:local_averaging_methods}
    ...
    \label{sub:quick_review}
        ...
        \begin{itemize}
            \item Binary Classification: $\mathcal{Y} = \{-1, 1\}$ and $l(y, \hat{y}) = \mathbbm{1}\{y \neq \hat{y}\}$, the Bayes predictor is equal to 
                    $$ f^*(X) \in \argmax_{y \in \{-1, 1\}} \ \mathbb{P}(Y = y \mid X) = \sign(\eta(X) - 1/2 ) $$
                where $\eta(X) = \mathbb{P}(Y = 1 \mid X)$。 This result extends naturally to multi-category classification with the Bayes predictor 
    ...
    \label{sub:local_averaging_methods}
        ...
        However, the conditional probability $\mathbb{P}(Y \mid X)$ is generally unknown. To overcome this obstacles, this time we approximate the $\mathbb{P}(Y \mid X)$ by some estimator $\hat{\mathbb{P}}(Y \mid X)$, and the optimal predictor could be obtained by
        \begin{equation*}
    ...
    \subsection{Linear Estimators} % (fold)
    \label{sub:linear_estimators}
        ...
        \end{equation*}
        These conditions ensure that for all $x \in \mathcal{X}$, $\hat{P}(Y \mid X)$ is a probability distribution. For our running examples, we have:
        \begin{itemize}
        ...
        \begin{figure}[h]
            \centering
            \includegraphics[width=.8\linewidth]{Pics/weight-functions.png}
        ...
        \label{ssub:partition_estimators}
            ...
            \vspace{1em}
            \noindent
            \textbf{Equivalence with Least-squares Regression}. Consider the case of regression where we use the linear estimator $\hat{f}(X) = \sum_{i=1}^{n} \hat{w}_i(X) Y_i $ with partition weights. This can be seen as a least-square estimator with feature vector $\varphi(X) = (\mathbbm{1}_{X \in A_1}, \dots, \mathbbm{1}_{X \in A_J})^{\top} \in \mathbb{R}^J$ in ERM approach. Indeed, from trainning data $(X_1, Y_1), \dots, (X_n, Y_n)$, we need to find the weight vector $\hat{\theta}$ through the normal equations
        ...
        \label{ssub:nadaraya_watson_estimator_kernel_regression_}
            ...
            \begin{itemize}
                \item Box kernel: $q(X) = \mathbbm{1}(||X||_2 \leq 1)$. See above for an illustration in $d = 2$ dimension
    ...
    % subsection linear_estimators (end)

    \subsection{Generic Consistency Analysis} % (fold)
    \label{sub:generic_consistency_analysis}

        We consider for simplicity the \textbf{regression} case. For classification, calibration techniques such as those used in Section \ref{sec:convexification_of_the_risk} can be used (with then a square root calibration function on top of the least-squares excess risk), but better rate can be obtained directly (please refer the book \cite{bach2021learning}).
        ...
        \begin{equation*}
            \begin{aligned}
                ...
                &= \underbrace{\sum_{i=1}^{n} \hat{w}_i(x) \cdot (Y_i - \mathbb{E}[Y_i \mid X_i])}_{\text{part I}} + \underbrace{\sum_{i=1}^{n} \hat{w}_i(x) \cdot \left(f^{*}(X_i) - f^{*}(x) \right)}_{\text{part II}} \\ 
            \end{aligned}
        \end{equation*}
        ...
        \begin{equation*}
            \begin{aligned}
                \mathbb{E}_{S_n}\left[\left(\hat{f}(x) - f^{*}(x) \right)^2 \mid X_1, \dots, X_n \right] &= \left(\mathbb{E}_{S_n} \left[\hat{f}(x) - f^*(x) \mid X_1, \dots, X_n \right] \right)^2 + \var_{S_n} \left(\hat{f}(x) - f^*(x) \mid X_1, \dots, X_n \right) \\
                &= \left(\mathbb{E}_{S_n}\Big[\text{part II} \mid X_1, \dots, X_n \Big] \right)^2 + \var_{S_n} \Big(\text{part I} \mid X_1, \dots, X_n \Big) \\
                &= \underbrace{\left(\sum_{i=1}^{n} \hat{w}_i(x) \cdot \big(f^*(X_i) - f^*(x) \big) \right)^2}_{\text{bias}} + \underbrace{\sum_{i=1}^{n} \hat{w}_i(x)^2 \cdot \mathbb{E}_{S_n} \Big[\big(Y_i - \mathbb{E}[Y_i \mid X_i] \big)^2 \mid X_i \Big]}_{\text{variance}} \\
            \end{aligned}
        \end{equation*}
        ...
        \begin{equation}
            \begin{aligned}
                ...
                &= \int_{\mathcal{X}} \mathbb{E}_{S_n}\Big[\big(\hat{f}(x) - f^*(x) \big)^2 \mid X = x \Big] d P(x) \\ 
        ...
        \end{equation}
        Notice that the expectation is with respect to the training data $S_n$, where the expectation with respect to the testing point $X$ is kept as an integral to avoid confusions.
        ...
        \begin{itemize}
            ...
            Hence, up to vanishing constant, the variance term measures the deviation t0 the uniform weights.
        \end{itemize}
        ...
        \subsubsection{Fixed Partition} % (fold)
        \label{ssub:fixed_partition}
            ...
            \end{proposition}
            There is a tradeoff between bias and variance, and we need to balance the terms (up to constants) $\max_{j \in J} \text{diam}(A_j)^2$ and $|J|/n$. 
            ...
            While optimal, this is a very slow rate, and a typical example of the curse of dimensionality. For this rate to be small, $n$ has to be exponentially large in dimension. This is unavoidable with so little regularity (only bounded first-order derivatives). In Section \ref{sec:kernel_methods}, we show how to leverage smoothness to get significantly improved bounds. In Section \ref{sec:sparse_methods}, we will leverage dependence on a small number of variables.
                    ...
                    \item \textbf{Bias}. We have, for $X \in A_j$ and a non-empty cell $A_j$,
                        ...
                        \begin{equation*}
                            \begin{aligned}
                                ...
                                &\leq \int_{\mathcal{X}} \sum_{j \in J} \mathbbm{1}(x \in A_j) \cdot \mathbb{E}_{S_n} \Big[\text{diam}(A_j)^2 \cdot \mathbbm{1}(n_{A_j} > 0) + \text{diam}(\mathcal{X})^2 \cdot \mathbbm{1}(n_{A_j} = 0) \Big] dP(x) \\
                                &= \sum_{j = 1}^{J} \mathbb{P}(A_j) \cdot \Big[\text{diam}(A_j)^2 \cdot \mathbb{P}(n_{A_j} > 0) + \text{diam}(\mathcal{X})^2 \cdot \mathbb{P}(n_{A_j} = 0) \Big] \\
        ...
        \label{ssub:k_nearest_neighbors}
            ...
            \begin{proof}
                Without loss of generality, we assume $2k \leq n$ (otherwise, the bound is trivial).
        ...
        \subsubsection{Kernel Regression} % (fold)
        \label{ssub:kernel_regression}
            ...
            \noindent
            \textbf{Smoothing by convolution}. When performing kernel smoothing, quantities like $\frac{1}{n} \sum_{i=1}^{n} q_h(x - X_i) g(X_i)$ naturally appear. When the number $n$ of observations goes to infinity, by the law of large numbers, it tends to almost surely to 
            ...
            \begin{figure}[h]
                \centering
                \includegraphics[width=.5\textwidth]{Pics/smoothing-convolution.png}
            \end{figure}

            We can now look at the generalization bound from Eq.(\ref{eq:expected-excess-risk-decomosition}) and see how it applies to kernel regression. We now consider the $l_2$-distance for simplicity, and consider the variance and bias term separately, first with an asymptotic result and then a formal result.

            \vspace{1em}
            \noindent
            \textbf{Variance term}. We have, for a fixed test point $x \in \mathcal{X}$:
            ...
            When $h$ tends to zero, then the denominator above $(q_h * p(x))^2$ tends to $p(x)^2$ because the bandwidth of the smoothing goes to zero ($q_h \rightarrow \delta(x)$). The numerator above corresponds to the smoothing of $p$ by the density $x \mapsto \frac{q_h(x)^2}{\int_{\mathbb{R}^d} q_h(u)^2 du}$ (where $\int_{\mathbb{R}^d} q_h(u)^2 du$ here is just a normalize constant), and is thus asymptotically equivalent to 
            ...
            \vspace{1em}
            \noindent
            \textbf{Bias term}. With the same intuitive reasoning, we get, when $n$ tends to infinity:
            ...
            \vspace{1em}
            \noindent
            \textbf{Overall result}. Combining the results from variance and bias terms, we get an \textit{asymptotic bound} of Eq.(\ref{eq:expected-excess-risk-decomosition}) proportional to (up to constants depending on $q$):
            ...
            \begin{proof}
                In order to deal with the denominator in the definition of the weights, we can firstly use Bernstein's inequality, applied to the random variables $q_h(X - X_i)$ which is almost surely in $[0, h^{-d} ||q||_{\infty}]$, to bound
                    \begin{equation*}
        ...
        % subsubsection kernel_regression (end)

    % subsection generic_consistency_analysis (end)

    \subsection{Universal Consistency} % (fold)
    \label{sub:universal_consistency}
        ...
        \end{itemize}
        These were enough to show consistency when the target function is Lipschitz-continuous in $\mathbb{R}^d$ as seen in Eq.(\ref{eq:expected-excess-risk-decomosition}). These also led to a precise rate of convergence (which turned out to be optimal).

        In order to show universal consistency for any square-intergrable functions, we need an extra (technical) assumption, which was first outlined in Stone's theorem, namely that there exists $c > 0$ such that for any non-negative integrable function $h: \mathcal{X} \mapsto \mathbb{R}$, 
        \begin{equation}
        ...
        \vspace{1em}
        \noindent
        \textbf{Proving universal consistency}. We can then substitue the upper-bound of bias term into the first inequality of Eq.(\ref{eq:empirical-excess-risk-decomposition}), which is
        \begin{equation}
            \begin{aligned}
                ...
                &\leq 3(c + 1) \varepsilon^2 + 3 B(\varepsilon)^2 \int_{\mathcal{X}} \mathbb{E}_{S_n} \left[\sum_{i=1}^{n} \hat{w}_i(x) d(X_i, x)^2 \right] dP(x) + \sigma^2 \int_{\mathcal{X}} \mathbb{E}_{S_n}\left[\sum_{i=1}^{n} \hat{w}_i(x)^2 \right] dP(x) \\
            \end{aligned}
        \end{equation}
        which is similar to the bound in Eq.(\ref{eq:expected-excess-risk-decomosition}). Next we can use the same tools for consistency as for Eq.(\ref{eq:expected-excess-risk-decomosition}).
        ...
        for a well chosen hyperparameter and number of observations $n$ as previous section. Thus, if the extra condition in Eq.(\ref{eq:technical-assumption-LAM}) is satisfied, these three methods are universally consistent. We can now look at the cases
        \begin{itemize}
    ...
    % subsection universal_consistency (end)

% section local_averaging_methods (end)

\clearpage

\section{Sparse Methods} % (fold)
\label{sec:sparse_methods}

% section sparse_methods (end)
...
\begin{appendices}
    ...
    \label{sec:probability_theory}
        ...
        \label{sub:expectations}
            ...
            \begin{corollary}[Law of Total Covariance]
                For random variable $X$, $Y$ and $Z$, 
                \begin{equation}
                    \cov(X, Y) = \mathbb{E}\big[ \cov(X, Y \mid Z) \big] + \cov \big( \mathbb{E}[X \mid Z], \mathbb{E}[Y \mid Z] \big)
                \end{equation}
            \end{corollary}
    ...
    \label{sec:concentration_of_measure}
        ...
        \label{sub:chernoff_s_methods}
            ...
            \noindent
            \textbf{Bounded Random Variables}. We are going to consider the case of bounded random variables and derive the so called Hoeffding's bound for them. As we know, the bounded random variables are the special case of sub-Gaussian variables.
        ...
        \label{sub:bernstein_s_inequality}
            ...
            \begin{theorem}[Berstein's Inequality]
                ...
                \begin{equation}
                    \mathbb{P}\left(\left| \frac{1}{n} \sum_{k=1}^{n} X_k - \mu \right| \geq t \right) \leq 2 \exp \left(- \frac{n t^2}{2\sigma^2 + 2(b - a)t/3 } \right)
                    \label{eq:Bernstein}
                \end{equation}
            \end{theorem}
            ...
            \begin{proof}
                ...
                \begin{equation*}
                    \begin{aligned}
                        ...
                        &= \exp \left(-\frac{n \sigma^2}{c^2} \Big( (1 + \alpha) \ln(1 + \alpha) - \alpha \Big) \right), \qquad \alpha = \frac{tc}{\sigma^2} = \frac{t(b - a)}{\sigma^2 } \\
                    \end{aligned}
                \end{equation*}
                ...
                we have
                \begin{equation*}
                    \mathbb{P}\left(|\bar{X}_n - \mu| \geq t \right) \leq \exp\left(- \frac{n t^2}{2 \sigma^2 + 2(b - a)t/3} \right)
                \end{equation*}
