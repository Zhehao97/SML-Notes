...
\label{sec:introduction_to_supervised_learning}
    ...
    \label{sub:statistical_learning_theory}
        ...
        An algorithm $\mathcal{A}$ is a mapping from $\mathcal{D}_{P}(n)$ (for any size $n$) to a function from $\mathcal{X}$ to $\mathcal{Y}$. The risk depends on the probability $P \in \mathcal{P}$, as $\mathcal{R}_P(f)$. The goal is to find $\mathcal{A}$ such that the risk
        \begin{equation*}
            \mathcal{R}_{P}\big(\mathcal{A}(\mathcal{D}_P(n) ) \big) - \mathcal{R}_P^* 
        \end{equation*}
        is small enough, where $\mathcal{R}_{P}^{*}$ is the Bayes risk. Here we assume that $\mathcal{D}_P(n)$ is sampled from $P$, where $P \in \mathcal{P}$ is unknown. Moreover, the risk is random because $\mathcal{D}(n)$ is random.
        ...
        \label{ssub:measures_of_performance}
            ...
            \begin{itemize}
                \item \textbf{Expected Error}: we measure the performance of algorithm as                  
                    \begin{equation*}
                        \mathbb{E}\Big[\mathcal{R}_{P}\big( \mathcal{A}(\mathcal{D}_n(P) ) \big) \Big]
                    \end{equation*}
                    where the expectation is with respect to the training data.
                    \begin{itemize}
                        \item Consistency: an algorithm $\mathcal{A}$ is called \textit{consistent in expectation} for the distribution $P$, if 
                            $$ \mathbb{E}\Big[\mathcal{R}_{P}\big( \mathcal{A}(\mathcal{D}_n(P) ) \big) \Big] - \mathcal{R}_{P}^{*} $$
                ...
                \item \textbf{Probably Approximately Correct (PAC) Learning}: for a given $\delta \in (0, 1)$ and $\varepsilon > 0$
                    \begin{equation*}
                        \mathbb{P}\Big( \mathcal{R}_P\big(\mathcal{A}(\mathcal{D}_n(P) ) \big) - \mathcal{R}_{P}^* \leq \varepsilon \Big) \geq 1 - \delta
                    \end{equation*}
                    The crux is to find $\varepsilon$ which is as small as possible (typically as a function of $\delta$). 
                    \begin{itemize}
                        \item Consistency: an algorithm $\mathcal{A}$ is called consistent in PAC for the distribution $P$, if for any $\varepsilon > 0$, there exists $\delta_n \in (0, 1)$, such that
                            $$ \mathbb{P}\Big( \mathcal{R}_P\big(\mathcal{A}(\mathcal{D}_n(P) ) \big) - \mathcal{R}_{P}^* \leq \varepsilon \Big) \geq 1 - \delta_n $$
        ...
        \label{ssub:some_notions_in_learning_problems}
            ...
            \begin{equation*}
                \sup_{P \in \mathcal{P}} \ \mathbb{E}\Big[\mathcal{R}_P \big(\mathcal{A}(\mathcal{D}_P(n) ) \big) \Big] - \mathcal{R}_{P}^{*}
            \end{equation*}
            ...
            \begin{definition}[Minimax Risk]
                ...
                \begin{equation}
                    \inf_{\mathcal{A}} \sup_{P \in \mathcal{P}} \ \mathbb{E}\Big[\mathcal{R}_P \big(\mathcal{A}(\mathcal{D}_P(n) ) \big) \Big] - \mathcal{R}_{P}^{*}
                \end{equation}
            \end{definition}
            The minimax risk is typlically a function of the sample size $n$, the properties of $\mathcal{X}, \mathcal{Y}$ and the distribution space $\mathcal{P}$. In order to compute the estimates of minimax risk, several techniques exist:
            \begin{itemize}
                \item \textbf{Upper-bounding} 

                    One given algorithm with a convergence proof provides an upper-bound on the optimal performance

                \item \textbf{Lower-bounding}
                    In some setups, it is possible to show that the infimum over all algorithms is greater than a certian quantity.
            \end{itemize}
        ...
        \label{ssub:no_free_lunch_theorems}
            ...
            \begin{theorem}[No Free Lunch - Fixed n]
                ...
                \begin{equation*}
                    \sup_{P \in \mathcal{P}} \ \mathbb{E}\Big[\mathcal{R}_{P}\big(\mathcal{A}(\mathcal{D}_P(n) ) \big) \Big] - \mathcal{R}_{P}^* \geq 1/2
                \end{equation*}
            \end{theorem}
            ...
            \begin{theorem}[No Free Lunch - Sequence of Errors]
                ...
                \begin{equation*}
                     \mathbb{E}\Big[\mathcal{R}_{P}\big(\mathcal{A}(\mathcal{D}_P(n) ) \big) \Big] - \mathcal{R}_{P}^* \geq a_n
                \end{equation*}
            \end{theorem}
...
\label{sec:empirical_risk_minimization}
    ...
    where $g$ is a measurable function. In this section, we consdier the methods based on empirical risk minimization.

    \subsection{Convexification of the Risk} % (fold)
    \label{sub:convexification_of_the_risk}
        ...
        Instead of learning $f: \mathcal{X} \rightarrow \{-1, 1\}$, we will thus learn a function $g: \mathcal{X} \rightarrow \mathbbm{R}$ and define $f(x) = \sign(g(x) )$ where
        \begin{equation*}
            \sign(a) = 
            \left\{ 
            \begin{aligned}
        ...
        where $\phi_{0-1}: \mathbb{R} \rightarrow \mathbb{R}$, with $\phi_{0-1}(u) = \mathbbm{1}(u < 0) $ is called the "margin-based" $0-1$ loss function. For empirical risk minimization, we then miniize the empirical risk
        $$ \hat{\mathcal{R}}(g) = \frac{1}{n} \sum_{i=1}^{n} \phi_{0-1}(y_i g(x_i) ) $$
        with respect to $g: \mathcal{X} \rightarrow \mathbb{R}$. However, the function $\phi_{0-1}$ is not continuous (and thus also non-convex) and leads to difficult optimization problems.

        \subsubsection{Convex Surrogates} % (fold)
        \label{ssub:convex_surrogates}
        ...
        % subsubsection convex_surrogates (end)

        \subsubsection{Geometric Interpretation of the Support Vector Machine} % (fold)
        \label{ssub:geometric_interpretation_of_the_support_vector_machine}
        ...
        % subsubsection geometric_interpretation_of_the_support_vector_machine (end)

        \subsubsection{Conditional Surrogate Risk and Classification Calibration} % (fold)
        \label{ssub:conditional_surrogate_risk_and_classification_calibration}
        ...
        % subsubsection conditional_surrogate_risk_and_classification_calibration (end)

        \subsubsection{Relationship between Risk and Surrogate Risk} % (fold)
        \label{ssub:relationship_between_risk_and_surrogate_risk}
        ...
        % subsubsection relationship_between_risk_and_surrogate_risk (end)

        \subsubsection{Impact on Approximation Errors} % (fold)
        \label{ssub:impact_on_approximation_errors}
        ...
        % subsubsection impact_on_approximation_errors (end)
    ...
    \label{sub:approximation_error}
        ...
        \begin{itemize}
            \item the second term $\inf_{\theta \in \mathbb{R}^d} \mathcal{R}(f_\theta) - \mathcal{R}^*$ is the incompressible error coming from the chosen of models $f_\theta$

            \item the first term $ \theta \mapsto \mathcal{R}(f_{\theta}) - \inf_{\theta \in \mathbb{R}^d} \mathcal{R}(f_{\theta}) $ is positive on $\mathbb{R}^d$, which can be typically upperbounded by a certain norm $\Omega(\theta - \theta_{*})$. Hence it represents a "distance" between minimizer $\theta_{*}$ and set $\Theta$ on $\mathbb{R}$
        \end{itemize}
...
\label{sec:pac_learn_and_uniform_convergence}
    ...
    \label{sub:pac_learning}
        ...
        \begin{definition}[PAC Learnability]
            ...
            $ m_{\mathcal{F}}: (0, 1)^2 \rightarrow \mathbb{N} $
        ...
        The function $m_{\mathcal{F}}: (0, 1)^2 \rightarrow \mathbb{N}$ determines the \textit{sample complexity} of learning $\mathcal{F}$: that is, how many examples are required to guarantee a probably approximately correct solution.
...
\label{sec:rademacher_complexity}
    ...
    \label{sub:motivation_for_rademacher_complexity}
        ...
        \end{definition}
        We have shown that if $S$ is an $\varepsilon/2$-representative sample, then the ERM rule is $\varepsilon$-consistent, namely, $\mathcal{R}(\hat{f}) - \inf_{f \in \mathcal{F}} \mathcal{R}(f) \leq \varepsilon $, where $\hat{f} = \text{ERM}_{\mathcal{F}}(S)$. 
        ...
        \end{equation}
        This can be written more compactly by defining $\sigma = (\sigma_1, \dots, \sigma) \in \{\pm 1\}^n$ to be a vector such that $S_1 = \{Z_: \sigma_i = 1 \}$ and $S_2 = \{Z_i: \sigma_i = -1 \}$. Then, if we further assume that $|S_1| = |S_2|$, then the equation above can be rewritten as
        \begin{equation}
    ...
    \label{sub:rademacher_complexity}
        ...
        $$ \hat{R}_{S}(\mathcal{H}) = \mathbb{E}_{\sigma} \left[\sup_{h \in \mathcal{H}} \frac{\langle \sigma, h_S \rangle }{n} \right] $$
        ...
        \begin{proof}
            Let $\mathcal{D}' = \{Z_1', \dots, Z_n'\}$ be an independent copy of the data $\mathcal{D} = \{Z_1, \dots, Z_n\}$. Let $(\sigma_i)_{i \in \{1, \dots, n\}}$ be i.i.d. Rademacher random variables, which are also independent of $\mathcal{D}$ and $\mathcal{D}'$. Using that for all $i \in \{1, \dots, n\}$, $\mathbb{E}[h(Z_i') \mid \mathcal{D} ] = \mathbb{E}[h(Z)]$, we have
            \begin{equation*}
                \begin{aligned}
                    \mathbb{E}\left[\sup_{h \in \mathcal{H}} \left(\mathbb{E}[h(Z)] - \frac{1}{n} \sum_{i=1}^{n}h(Z_i) \right) \right] &= \mathbb{E}\left[\sup_{h \in \mathcal{H}} \left(\frac{1}{n} \sum_{i=1}^{n} \mathbb{E}[h(Z_i') \mid \mathcal{D}] - \frac{1}{n} \sum_{i=1}^{n} h(Z_i) \right) \right] \\
                    &= \mathbb{E}\left[\sup_{h \in \mathcal{H}} \left(\frac{1}{n} \sum_{i=1}^{n} \mathbb{E}[h(Z_i') - h(Z_i) \mid \mathcal{D} ]  \right) \right] \\
                \end{aligned}
            \end{equation*}
            ...
            \begin{equation*}
                \begin{aligned}
                    \mathbb{E}\left[\sup_{h \in \mathcal{H}} \left(\mathbb{E}[h(Z)] - \frac{1}{n} \sum_{i=1}^{n}h(Z_i) \right) \right] &\leq \mathbb{E}\left[\mathbb{E}\left( \sup_{h \in \mathcal{H}} \left(\frac{1}{n} \sum_{i=1}^{n}[h(Z_i') - h(Z_i) ] \right) \mid \mathcal{D} \right) \right] \\
        ...
        \begin{theorem}[Generalization bonud via Rademacher Complexity]
            ...
            \begin{equation}
                \begin{aligned}
                    \mathbb{E}[h(Z)] &\leq \frac{1}{n} \sum_{i=1}^n h(Z_i) + 2 R_{n}(\mathcal{H}) + \sqrt{\frac{\log (w / \delta) }{2n}} \\
        ...
        \begin{proof}
            ...
            \begin{itemize}
                ...
                \item Step 2. We apply the symmetrization theorem \ref{thm:Rademacher-complexity-symmetrization} to get the upper bound of the expectation of the empirical process 
                \begin{equation*}
                ...
                \item Step 3. Bound expected Rademacher complexity through empirical Rademacher complexity and McDiarmid inequality. To begin with, define
                    $$ \tilde{H} = \hat{R}_{S}(\mathcal{H}) := \mathbb{E}_{\sigma} \left[\sup_{h \in \mathcal{H}} \frac{1}{n} \sum_{i=1}^{n} \sigma_i h(Z_i) \right] $$
                ...
                \item Step 4. Putting all things together by noticing that
                    $$ \mathbb{E}[\tilde{H}] = \mathbb{E}_{P}[\hat{R}_{S}(\mathcal{H})] = R_n(\mathcal{H}) $$
                    ...
                    \begin{equation*}
                        \begin{aligned}
                            \mathbb{E}[h(Z)] &\leq \frac{1}{n} \sum_{i=1}^n h(Z_i) + 2 R_{n}(\mathcal{H}) + \sqrt{\frac{\log (1 / \delta) }{2n}} \\
                            \mathbb{E}[h(Z)] &\leq \frac{1}{n} \sum_{i=1}^n h(Z_i) + 2 \hat{R}_{S}(\mathcal{H}) + 3 \sqrt{\frac{\log (2 / \delta) }{2n}} \\
                        \end{aligned}
                    \end{equation*}
            \end{itemize}
        ...
        \begin{proposition}
            Let $\mathcal{H}$ be a family of functions mapping $\mathcal{Z} \rightarrow \mathbb{R}$ and define $\mathcal{H}' = \{h'(Z) = h(Z) + c_0 \mid h \in \mathcal{H} \}$ for some $c_0 \in \mathbb{R}$. Then we have 
    ...
    \label{sub:lipschitz_continuous_losses}
        ...
        \begin{proposition}[Contraction Principle - Lipschitz-continuous Functions]
            Given any functions $b$, $a_i : \Theta \rightarrow \mathbb{R}$ (no assumption on $\Theta$) and $\varphi_i: \mathbb{R} \rightarrow \mathbb{R}$ any $1$-Lipschitz-functions, for $i = 1, \dots, n$, we have, for $\sigma \in \mathbb{R}^n$ a vector of independent Rademacher random variables,
            \begin{equation}
        ...
        \begin{equation}
            \mathbb{E}_{\sigma} \left[\sup_{f \in \mathcal{F}} \frac{1}{n} \sum_{i=1}^{n} \sigma_i l(Y_i, f(X_i) ) \mid \mathcal{D} \right] \leq G \cdot \mathbb{E}_{\sigma} \left[\sup_{f \in \mathcal{F}} \frac{1}{n} \sum_{i=1}^{n} \sigma_i f(X_i) \mid \mathcal{D} \right] 
        \end{equation} 
...
\label{sec:covering_number_and_chaining}
    ...
    \label{sub:covering_and_packing}
        ...
        We begin by defining the notions of packing and covering a set in a metric space. Recall that a metric space $(T, \rho)$ consists of a non-empty set $T$, equipped with a mapping $\rho: T \times T \rightarrow \mathbb{R}$ that satisfies the following properties:
        \begin{enumerate}[(a)]
...
\label{sec:norms}
    ...
    \label{sub:equivalence_of_norms}
        ...
        \textbf{We conclude that any norms on all finite-dimensional vector space are equivalent, but on infinite-dimensional vector spaces, the result need not hold.}
