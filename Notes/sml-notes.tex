...
\label{sec:rademacher_complexity}
    ...
    \label{sub:rademacher_complexity}
        ...
        \begin{proposition}[Translation Invariant]
...
\label{sec:optimization_for_machine_learning}
    ...
    \label{sub:optimization_in_machine_learning}
        ...
        In the following content, we use the notation $\theta^{*}$ for the minimizer of the expected risk and the notation $\theta^{\star}$ (not $\hat{\theta}$) for the minimizer of the empirical (potentially regularized) risk.
    ...
    \label{sub:gradient_descent_on_smooth_problems}
        ...
        \label{ssub:analysis_of_gd_for_ordinary_least_squares}
            ...
            We first start with the simplest example, namely the quadratic convex functions (which is strongly convex), which is just the loss function in ordinary least squares. Let $\Phi \in \mathbb{R}^{n \times d}$ be the design matrix, and $y \in \mathbb{R}^n$ the vector of responses. Least-squares estimation amoutns to finding a minimizer $\theta^{\star}$ of

            \begin{equation*}
                F(\theta) = \frac{1}{2n} ||\Phi \theta - y||_2^2 = \frac{1}{2n} (y - \Phi \theta)^{\top} (y - \Phi \theta)
            \end{equation*}
            The gradient of $F$ is $F'(\theta) = \frac{1}{n} \Phi^{\top}(\Phi \theta - y) = \frac{1}{n} \Phi^{\top} \Phi \theta - \frac{1}{n} \Phi^{\top} y $. Thus, denoting $H = \frac{1}{n} \Phi^{\top} \Phi \in \mathbb{R}^{d \times d}$ the Hessian matrix (with respect to $\theta$), the minimizers $\theta^{\star}$ are characterized by
            \begin{equation}
                F'(\theta^{\star}) = 0 \quad \Rightarrow \quad H \theta^{\star} = \frac{1}{n} \Phi^{\top} y
            \end{equation}
            Since $\frac{1}{n} \Phi^{\top}y \in \mathbb{R}^d$ is in the range space of $H$, there is always a minimizer, but unless $H$ is invertible, the minimizer is not unique. But all minimizers $\theta^{\star}$ have the same function value $F(\theta^{\star})$, and we have, from a simple Taylor expansion
            \begin{equation*}
                \begin{aligned}
                    F(\theta) - F(\theta^{\star}) &= F'(\theta^{\star})^{\top}(\theta - \theta^{\star}) + \frac{1}{2} (\theta - \theta^{\star})^{\top} H (\theta - \theta^{\star}) \\ 
            ...
            \begin{equation*}
                \begin{aligned}
                    ...
                    &= \theta_{t-1} - \gamma H(\theta_{t-1} - \theta^{\star}) \\
                \end{aligned}
            \end{equation*}
            leading to 
            \begin{equation*}
                \theta_t - \theta^{\star} = \theta_{t - 1} - \theta^{\star} - \gamma H(\theta_{t - 1} - \theta^{*}) = (I - \gamma H) (\theta_{t - 1} - \theta^{\star})
            \end{equation*}
            that is, we have linear recursion, and we can unroll the recursion and write
            \begin{equation*}
                \theta_t - \theta^{\star} = (I - \gamma H)^{t} (\theta_0 - \theta^{\star})
            \end{equation*}
            We can now look at various measures of performace:
            \begin{equation*}
                \begin{aligned}
                    ||\theta_t - \theta^{\star}||_2^2 &= (\theta_0 - \theta^{\star})^{\top} (I - \gamma H)^{2t} (\theta_0 - \theta^{\star}) \\
                    F(\theta_t) - F(\theta^{\star}) &= \frac{1}{2} (\theta_0 - \theta^{\star} )^{\top} (I - \gamma H)^{2t} H (\theta_0 - \theta^{\star})
                \end{aligned}
            \end{equation*}
            Note that the second equality holds by applying $H \theta^{\star} = \frac{1}{n} \Phi^{\top} y$.
        ...
        \label{ssub:analysis_of_gd_for_strongly_and_smooth_functions}
            ...
            \begin{definition}[Convex function]
                ...
                \begin{equation}
                    ...
                    \label{eq:convexity}
                \end{equation}
                this conclusion holds even for $\eta = \theta^{\star}$.
            \end{definition}
            ...
            \begin{lemma}[Lojasiewics inequality]
                If $F$ is differentiable and $m$-strongly convex with unique minimizer $\theta^{\star}$, then we have:
                \begin{equation*}
                    || F'(\theta) ||_2^2 \geq 2 \mu (F(\theta) - F(\theta^{\star}))
                \end{equation*}
                \label{lm:lojasiewics-inequality}
            \end{lemma}

            \begin{proof}
                The right-hand side of Eq.(\ref{eq:strong-convexity}) is strongly convex in $\eta$ and minimized with $\eta = \theta - \frac{1}{\mu } F'(\theta)$. Plugging this value into the bound and letting $\eta = \theta^{\star}$, we get
                \begin{equation*}
                    F(\theta^{\star}) \geq F(\theta) - \frac{1}{\mu} ||F'(\theta)||_2^2 + \frac{1}{2 \mu} ||F'(\theta)||_2^2 = F(\theta) - \frac{1}{2 \mu} ||F'(\theta)||_2^2
                \end{equation*}
            ...
            \begin{theorem}[Convergence of GD for smooth strongly-convex functions]
                ...
                \begin{equation*}
                    F(\theta_t) - F(\theta^{\star}) \leq \left(1 - \frac{\mu}{L} \right)^t \cdot (F(\theta_0) - F(\theta^{\star}) ) \leq e^{- t \mu / L} \cdot (F(\theta_0) - F(\theta^{\star} ))
                \end{equation*}
            \end{theorem}
            ...
            \begin{proof}
                ...
                \end{equation}
                Adding $-F(\theta^{\star})$ on both sides, we get
                \begin{equation*}
                    F(\theta_t) - F(\theta^{\star}) \leq (F(\theta_{t-1}) - F(\theta^{\star})) - \frac{1}{2L} || F'(\theta_{t-1}) ||_2^2
                \end{equation*}
                Using Lojasiewicz inequality (Lemma \ref{lm:lojasiewics-inequality}), it follows
                \begin{equation*}
                    F(\theta_t) - F(\theta^{\star}) \leq (1 - \mu/L) \cdot (F(\theta_{t-1}) - F(\theta^{\star}) ) \leq e^{- \mu/L} \cdot (F(\theta_{t-1}) - F(\theta^{\star}) )
                \end{equation*}
        ...
        \label{ssub:analysis_of_gd_for_convex_and_smooth_functions}
            ...
            \begin{theorem}[Convergence of GD for smooth convex functions]
                Assume that $F$ is $L$-smooth and convex, with a global minimizer $\theta^{\star}$. Choosing step-size $\gamma_t = 1/L$, the iterates $(\theta_t)_{t \geq 0}$ of Gradient Descent on $F$ satisfy
                \begin{equation*}
                    F(\theta_t) - F(\theta^{\star}) \leq \frac{L}{2t} ||\theta_0 - \theta^{\star} ||_2^2
                \end{equation*}
            \end{theorem}
            ...
            \begin{proof}
                ...
                \begin{equation}
                    V_t(\theta_t) = t (F(\theta_t) - F(\theta^{\star})) + \frac{L}{2}||\theta_t - \theta^{\star}||_2^2    
                \end{equation}
                and our goal is to show that it decays along iterations. We can split the difference in Lyapunov functions in three terms
                \begin{equation*}
                    V_t(\theta_t) - V_{t-1}(\theta_{t-1}) = \underbrace{t F(\theta_t) - t F(\theta_{t-1}) }_{\text{part I}} + \underbrace{ F(\theta_{t-1}) - F(\theta^{\star}) }_{\text{part II}} + \underbrace{ \frac{L}{2} ||\theta_t - \theta^{\star}||_2^2 - \frac{L}{2}||\theta_{t-1} - \theta^{\star}||_2^2 }_{\text{part III}}
                \end{equation*}
                ...
                \begin{itemize}
                    ...
                    \item For a convex function, we have
                        \begin{equation*}
                            F(\theta^{\star}) \geq F(\theta_{t-1}) + F'(\theta_{t-1})^{\top} (\theta^{\star} - \theta_{t-1}) \quad \Rightarrow \quad F(\theta_{t-1}) - F(\theta^{\star}) \leq F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta^{\star})
                        \end{equation*}

                    \item Expanding the difference of square, we have
                        \begin{equation*}
                            \begin{aligned}
                                ||\theta_{t} - \theta^{\star}||_2^2 - ||\theta_{t-1} - \theta^{\star}||_2^2 &= ||\theta_{t-1} - \gamma_t - \theta^{\star} ||_2^2 - - ||\theta_{t-1} - \theta^{\star}||_2^2 \\
                                &= - 2 \gamma_t F'(\theta_{t-1})^{\top}(\theta_{t-1} - \theta^{\star} ) + \gamma_t^2 ||F'(\theta_{t-1})||_2^2
                            \end{aligned}
                        \end{equation*}
                \end{itemize}
                ...
                \begin{equation*}
                    \begin{aligned}
                        V_t(\theta_t) - V_{t-1}(\theta_{t-1}) &\leq - \frac{t}{2L} ||F'(\theta_{t-1})||_2^2 + F'(\theta_{t-1})^{\top}(\theta_{t-1} - \theta^{\star}) - F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta^{\star}) + \frac{1}{2L} ||F'(\theta_{t-1}) ||_2^2 \\
                        &= \frac{1 - t}{2L} ||F'(\theta_{t-1})||_2^2 \leq 0
                    \end{aligned}
                \end{equation*}
                which leads to
                \begin{equation*}
                    t(F(\theta_t) - F(\theta^{\star}) ) \leq V_t(\theta_t) \leq V_0(\theta_0) = \frac{L}{2} ||\theta_0 - \theta^{\star}||_2^2
                \end{equation*}
                and thus $F(\theta_t) - F(\theta^{\star}) \leq \frac{L}{2t} ||\theta_0 - \theta^{\star}||_2^2$.
        ...
        \label{ssub:beyond_gradient_descent}
            ...
            \begin{itemize}
                \item \textbf{Nesterov acceleration}. For convex functions, a simple modification of gradient descent allows to obtain better convergence rates. The algorithm is as follows:
                    ...
                    \begin{equation*}
                        F(\theta_t) - F(\theta^{\star}) \leq \frac{2L ||\theta_0 - \theta^{\star}||_2^2 }{(t + 1)^2}
                    \end{equation*}
                    ...
                    \begin{equation*}
                        F(\theta_t) - F(\theta^{*}) \leq L(1 - \sqrt{\mu / L})^t ||\theta_0 - \theta^{\star}||_2^2
                    \end{equation*}
    ...
    \label{sub:gradient_methods_on_non_smooth_problems}
        ...
        \begin{equation*}
            \theta_t = \theta_{t-1} - \gamma_t G_t, \quad G_t \in \partial F(\theta_{t-1})
        \end{equation*}
        ...
        \begin{theorem}[Convergence of the subgradient method]
            Assume that $F$ is convex, $B$-Lipschitz-continuous, and admits a minimizer $\theta^{\star}$ that satisfies $||\theta^{\star} - \theta_0||_2 \leq D$. By choosing $\gamma_t = \frac{D}{B \sqrt{t}}$ then the iterates $(\theta_t)_{t \geq 0}$ of Subgradient Method on $F$ satisfy
            \begin{equation*}
                \min_{0 \leq s \leq t-1} \ F(\theta_s) - F(\theta^{\star}) \leq DB \frac{2 + \log t}{2 \sqrt{t}}
            \end{equation*}
        \end{theorem}

        \begin{proof}
            We look at how $\theta_t$ approaches $\theta^{\star}$, that is, we try to use $||\theta_t - \theta^{\star}||_2^2$ as a Lyapunov function. We have:
            \begin{equation*}
                \begin{aligned}
                    ||\theta_t - \theta^{\star}||_2^2 &= ||\theta_{t-1} - \gamma_t F'(\theta_{t-1}) - \theta^{\star}||_2^2 \\
                    &= ||\theta_{t-1} - \theta^{\star}||_2^2 - 2 \gamma_t F'(\theta_{t-1})^{\top} (\theta_{t-1} - \theta^{\star}) + \gamma_t^2 ||F'(\theta_{t-1]})||_2^2
                \end{aligned}
            \end{equation*}
            Combining this with the convexity inequality $F(\theta_{t-1}) - F(\theta^{\star}) \leq F'(\theta_{t-1})^{\top}(\theta_{t-1} - \theta^{\star})$ from Eq.(\ref{eq:convexity}), we have (also using the boundedness of gradients/Lipschiz-continuous)
            \begin{equation*}
                ||\theta_t - \theta^{\star}||_2^2 \leq ||\theta_{t-1} - \theta^{\star}||_2^2 - 2 \gamma_t [F(\theta_{t-1}) - F(\theta^{\star}) ] + \gamma_t^2 B^2
            \end{equation*}
            and thus, by isolating the distance to optimum in function values:
            \begin{equation}
                \gamma_t (F(\theta_{t-1}) - F(\theta^{\star}) ) \leq \frac{1}{2} \left(||\theta_{t-1} - \theta^{\star}||_2^2 - ||\theta_t - \theta^{\star}||_2^2 \right) + \frac{1}{2} \gamma_t^2 B^2
            \end{equation}
            It is sufficient to sum these inequalities up to $t$ and drop the term $-||\theta_{t+1} - \theta^{\star}||_2^2$ to get, for any $\theta^{\star} \in \mathbb{R}^d$,
            \begin{equation*}
                \frac{1}{\sum_{s=1}^{t} \gamma_s} \sum_{s=1}^{t} \gamma_s (F(\theta_{s-1}) - F(\theta^{\star}) ) \leq \frac{||\theta_0 - \theta^{\star}||_2^2}{2 \sum_{s=1}^{t} \gamma_s} + B^2 \frac{\sum_{s=1}^{t} \gamma_s^2}{2 \sum_{s=1}^{t} \gamma_s}
            \end{equation*}
            The left-hand side is trivially larger than $\min_{0 \leq s \leq t-1} (F(\theta_s) - F(\theta^{\star}) ) $, and than $F(\theta_t) - F(\theta^{\star})$ where $\bar{\theta}_t = (\sum_{s=1}^{t} \gamma_s \theta_{s-1}) / (\sum_{s=1}^{t} \gamma_s) $ by Jensen's inequality.
            ...
            we get the bound
            \begin{equation*}
                \min_{0 \leq s \leq t-1} (F(\theta_s) - F(\theta^{\star}) ) \leq \frac{1}{2 \tau \sqrt{t}} \left(D^2 + \tau^2 B^2 (1 + \log t) \right)
            \end{equation*}
